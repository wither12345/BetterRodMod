...
public class StickyHookRenderer extends EntityRenderer<@NotNull StickyHookEntity, @NotNull FishingHookRenderState> {
    private static final Identifier TEXTURE_LOCATION = Identifier.parse("better_rod:textures/entity/sticky_hook.png");
    private static final RenderType RENDER_TYPE = RenderTypes.entityCutoutCull(TEXTURE_LOCATION);

    public StickyHookRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    public boolean shouldRender(StickyHookEntity entity, @NotNull Frustum culler, double camX, double camY, double camZ) {
    ...
    public void extractRenderState(StickyHookEntity entity, FishingHookRenderState state, float partialTicks) {
    ...
    protected boolean affectedByCulling(StickyHookEntity entity) {
        return false;
    }
}
